...
import { applyToJob, ApplicationResponse } from '../../api/applications'; 

// ---------------------------------------------------
// Definición de Tipos de Datos (Solo para la oferta)
// ---------------------------------------------------
...
interface JobOffer {
  id: number;
  title: string;
// ... (resto de tu interfaz JobOffer)
  companyName: string; 
  role: "SETTER" | "CLOSER" | "BOTH" | "COLD_CALLER"; 
  model: string; 
  salaryHint: string; 
  type: string; 
  description: string;
  callTool: "calendly" | "zoom" | "whatsapp";
  callLink: string; // URL a Calendly, Zoom, etc.
}

// ---------------------------------------------------
// COMPONENTE AUXILIAR: Tarjeta de Oferta (Maneja el estado de aplicación)
// ---------------------------------------------------
const JobOfferCard: React.FC<{ offer: JobOffer }> = ({ offer }) => {
    // Estado local para manejar si ya aplicó, la carga y posibles errores
    const [loading, setLoading] = useState(false);
    const [applied, setApplied] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const handleApply = async () => {
        // Si ya aplicó (ej. después de una recarga), simplemente abrimos el enlace
        if (applied) {
        ...
        try {
            // 🚨 1. Llama al Backend para registrar la aplicación
            const result: ApplicationResponse = await applyToJob(offer.id); 
            
            // 2. Si es exitoso, marcamos como aplicado
            setApplied(true);
            
            // 3. Abrimos el enlace de la llamada (paso final y deseado)
            window.open(offer.callLink, "_blank", "noopener,noreferrer");
            ...
            if (err.response) {
                if (err.response.status === 409) {
                    // Conflicto: el backend nos dice que ya aplicamos
                    setError("Ya habías aplicado a esta oferta. Abriendo enlace...");
                    setApplied(true); // Actualizamos el estado para que el botón muestre el estado final
                    window.open(offer.callLink, "_blank", "noopener,noreferrer");
                } else {
                    // Otros errores del servidor (ej. 404 Not Found, 500 Internal Server Error)
                    setError(`Error del servidor: ${err.response.statusText}`);
                }
            } else {
                // Error de red (ej. Backend caído)
                setError("Error de conexión con el servidor. ¿Está el backend activo?");
            }
        } finally {
            setLoading(false);
        }
    ...
    // Clases dinámicas para el estado del botón
    ...
    return (
        ...
        >
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                {/* ... (Tu código de visualización de título, compañía, etc.) ... */}
                <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pt-2 border-t border-neutral-100">
                ...
                <div className="flex gap-2">
                    ...
                    {/* 🚨 REEMPLAZAMOS EL BOTÓN POR EL NUEVO CON LÓGICA DE APLICACIÓN */}
                    <Button
                        onClick={handleApply}
                        disabled={loading}
                        isLoading={loading}
...
// ---------------------------------------------------
// COMPONENTE PRINCIPAL: RepOffersPage
// ---------------------------------------------------
...
export default function RepOffersPage() {
    ...
    // ---------------------------------------------------
    // Manejo de Estado (Loading y Error) - Se mantiene igual
    // ---------------------------------------------------
    ...
    // ---------------------------------------------------
    // Renderizado con Datos - Usamos el nuevo componente JobOfferCard
    // ---------------------------------------------------
    ...
    return (
        <>
            ...
            <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                {/* Filtros por rol (Tu código de filtros es correcto) */}
